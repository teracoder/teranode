...
func (a *AddrManager) updateAddress(netAddr, srcAddr *wire.NetAddress) {
	...
	if len(a.addrNew[bucket]) > newBucketSize {
		a.logger.Infof("new bucket is full, expiring old")
		a.expireNew(bucket)
	}
...
func (a *AddrManager) Start() {
	...
	a.logger.Infof("Starting address manager")
...
func (a *AddrManager) Stop() error {
	if atomic.AddInt32(&a.shutdown, 1) != 1 {
		a.logger.Warnf("Address manager is already in the process of " +
			"shutting down")
		return nil
	}
...
func (a *AddrManager) Good(addr *wire.NetAddress) {
	...
	a.logger.Infof("Replacing %s with %s in tried", rmkey, addrKey)
...
func (a *AddrManager) GetBestLocalAddress(remoteAddr *wire.NetAddress) *wire.NetAddress {
	...
	if bestAddress != nil {
		...
		a.logger.Debugf("No worthy address for %s:%d", remoteAddr.IP, remoteAddr.Port)
