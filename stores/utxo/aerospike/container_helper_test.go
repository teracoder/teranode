...
var (
	// customize the container image if needed
	aerospikeContainerImage = "" // "aerospike:custom"
...
func initAerospike(t *testing.T, settings *settings.Settings, logger ulogger.Logger) (*uaerospike.Client, *teranode_aerospike.Store, context.Context, func()) {
	...
	func() {
		...
		aerospikeContainerOptions := []testcontainers.ContainerCustomizer{
			aeroTest.WithTTLSupport(aerospikeNamespace),
		}

		if aerospikeContainerImage != "" {
			aerospikeContainerOptions = append(aerospikeContainerOptions, aeroTest.WithImage(aerospikeContainerImage))
		}

		c, e := aeroTest.RunContainer(ctx, aerospikeContainerOptions...)
		if e != nil {
			err = e
			return
		}
